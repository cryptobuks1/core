...
@section('content')
    ...
    <div class="create-broker" style=" background-color: #00aced; border-radius: 10px">
        <div class="broker-top" style="margin: 10px; padding-top: 5px">
            <h2>TÌM CHUYẾN BAY</h2>
        </div>
        <div id="main-broker" class="container" style=" ">
            <div class="content-wap" style="margin: 10px">
                @include('layouts.errors')
                {!! Form::open(array('route' => 'front.flight.list','method'=>'POST','enctype' => 'multipart/form-data')) !!}
                    <div class="row">
                        <div class="col-xs-6 ">
                            <div class="form-group">
                                <label>Điểm khởi hành(<span class="red">*</span>):</label>
                                <select id="departure" class="form-control" name="StartPoint" required>
                                    <option value="">Điểm khởi hành</option>
                                    @foreach($stations as $item)
                                        <option value="{{$item->code}}" @if(app("request")->input("StartPoint")== $item->code) selected="selected" @endif>{{$item->city_vi.' ('.$item->name.')'}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-xs-6">
                            <div class="form-group">
                                <label  style="display: block">Điểm đến(<span class="red">*</span>):</label>
                                <select id="arrival" class="form-control" name="EndPoint" required>
