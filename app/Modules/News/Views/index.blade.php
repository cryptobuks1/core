...
<div class="card">
  ...
  <div class="card-header" style="border-bottom: 0">
    ...
    <div class="float-right" style="margin-right: 350px">
      <a href="{{ url($backendUrl.'/news/create') }}"><button class="btn btn-success"><i class="fa fa-plus-circle"></i> Thêm mới</button></a>
    </div>
    ...
    <table id="example1" class="table table-bordered table-striped dataTable">
      ...
      @foreach( $news as $post )
        <tr>
          ...
          <td><a href="{{url('tin-tuc/'.$post->news_slug)}}" target="_blank">{{ $post->title }}</a></td>
          <td>{{ $post->author }}</td>
          <td>{{ $post->language }}</td>
          <td>
              @if($post->status)
