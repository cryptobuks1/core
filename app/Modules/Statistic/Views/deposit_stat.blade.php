                            ...
                            <div class="input-group input-group-sm dataTables_filter" style="">
                                <form action="" name="formSearch" method="GET" >
                                    <div class="input-group">
                                        ...
                                        <select name="paygate_code" class="form-control" style="">
                                            <option value="" @if(app("request")->input("status")== "") selected="selected" @endif>Cổng thanh toán</option>
                                            @if($paygates && count($paygates) > 0)

                                                @foreach($paygates as $pg)
                                                <option value="{{$pg}}" @if(app("request")->input("paygate_code")== $pg) selected="selected" @endif>{{$pg}}</option>
                                                @endforeach
                                            @endif

                                        </select>
                                        <select name="group_user" class="form-control" style="">
                                            <option value="" @if(app("request")->input("group_user")== "") selected="selected" @endif>Nhóm khách hàng</option>
                                            @if($group_user && count($group_user) > 0)

                                                @foreach($group_user as $group)
                                                    <option value="{{$group->id}}" @if(app("request")->input("group_user")== $group->id) selected="selected" @endif>{{$group->name}}</option>
                                                @endforeach
                                            @endif
                        ...
                        <div class="row"><div class="col-sm-12">
                                <div class="table-responsive">
                                    <table id="example1" class="table table-bordered table-striped dataTable">
                                        ...
                                        @else
                                            @foreach( $deposits as $s_order )
                                                <tr>
                                                    <td>{{ $s_order->order_code }}</td>
                                                    <td>{!! App\User::getUserInfo($s_order->payee_id) !!}</td>
                                                    <td>{{ number_format($s_order->net_amount) }} {{ $s_order->currency_code }}</td>
                                                    <td><strong>{!! $s_order->paygate_code !!} @if($s_order->bank_code)({{$s_order->bank_code }}) @endif </strong><br>
                                                         <span class="text-success">{{$s_order->description}}</span>
                                                    </td>
                                                    <td>
