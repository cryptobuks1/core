...
<div class="card">
    <div class="card-header" style="border-bottom: 0">
        <div class="row">
            <div class="col-md-12">
                <h3 class="card-title">Lịch trình {{$tour->name}}</h3>
            </div>
        </div>
        <div class="float-right" style="margin-right: 150px">
            <a href="{{ url($backendUrl.'/tour/schedules/create/'.$tour->id) }}"><button class="btn btn-success"><i class="fa fa-plus-circle"></i> Thêm</button></a> |
      ...
      @if(count($datas)>0)
        ...
        <tr>
          ...
          <td>{!! $data->description !!}</td>
            <td>
               <div data-table="tour_schedules" data-id="{{ $data->id }}" data-col="status" class="Switch Round @if($data->status == 1) On @else Off @endif " style="vertical-align:top;margin-left:10px;">
            ...
            <div class="action-buttons" >
                <span style="display: flex">
                   <a href="{{ url($backendUrl.'/tour/schedules/edit/'.$data->id) }}"> <i title="Sửa" class="ace-icon fa fa-pencil bigger-130 btn btn-warning"></i> </a>  ||
                <a href="#" name="{{ $data->name }}" link="{{ url($backendUrl.'/tour/schedules/delete/'.$data->id) }}" class="deleteClick red id-btn-dialog2 btn btn-danger"data-toggle="modal" data-target="#deleteModal" > <i title="Delete" class="ace-icon fa fa-trash-o bigger-130"></i></a>
                </span>
            </div>
          </td>
        </tr>
