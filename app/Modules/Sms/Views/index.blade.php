...
<div class="card">
  ...
  <div class="card-header" style="border-bottom: 0">
    ...
    <div class="float-right" style="margin-right: 150px">
      <a href="{{ url($backendUrl.'/sms/provider') }}"><button class="btn btn-success"><i class="fa fa-plus-circle"></i> Cấu hình</button></a>
    ...
    <table id="example1" class="table table-bordered table-striped dataTable">
      ...
      @foreach( $smss as $sms )
        <tr>
          ...
          <td>{{ $sms->text }}</td>
          <td>{{ $sms->modulename }}</td>
          <td>{{ $sms->model_id}}</td>
          <td>{{ $sms->type}}</td>
           <td>{{ $sms->created_at}}</td>
          <td>{{ $sms->expired_at}}</td>
          <td>
            <div class="action-buttons">
          <a href="#" name="{{ $sms->text }}" link="{{ url($backendUrl.'/sms/'.$sms->id) }}" class="deleteClick red id-btn-dialog2"data-toggle="modal" data-target="#deleteModal" > <i title="Delete" class="ace-icon fa fa-trash-o bigger-130"></i></a>
    ...
    <div class="row">
      ...
      <div class="col-sm-12 col-md-7">
        <div class="float-right" id="dynamic-table_paginate">
          <?php $smss->setPath('sms'); ?>
          <?php echo $smss->render(); ?>
        </div>
      </div>
    </div>
  </div></form>
